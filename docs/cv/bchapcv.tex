\documentclass[margin,line]{resume}
 
\usepackage[utf8]{inputenc}
\usepackage[english,french,russian]{babel}
\usepackage[T1]{fontenc}
\usepackage{fontawesome}
\usepackage{graphicx,wrapfig}
\usepackage{url}
\usepackage{comment}
\usepackage{CJKutf8}
\usepackage[dvipsnames]{xcolor}		
\usepackage[colorlinks=true, pdfstartview=FitV, linkcolor=Blue, citecolor=Blue, urlcolor=Blue]{hyperref}
\usepackage[shortlabels]{enumitem}
%\pdfcompresslevel=9
\usepackage[pages=some]{background}

\backgroundsetup{
  scale=1,angle=0,color=black,
  contents={%
  }
}


\begin{document}{\Large \textbf{Brandon Chaperon}}
\begin{resume}

	% === PERSONAL INFO ===

	\section{\mysidestyle Email \vspace{0.3em}\\Website}
	%    Tibault Reveyrand \\
	%    Limoges, France\\ 
	\href{mailto:brandon.chaperon@mail.mcgill.ca}{%
		brandon.chaperon@mail.mcgill.ca
	}\vspace{0.3em}\\
	\href{https://bchaper0n.github.io/}{bchap.ca}
	%{}
	%    \faTv  \space \href{http://www.microwave.fr}{www.microwave.fr} \\ 
	%    \faTwitter  \space \href{https://twitter.com/reveyrand/}{@reveyrand} \\
	%    \faLinkedin \space \href{http://www.linkedin.com/in/reveyrand/}{www.linkedin.com/in/reveyrand} \\ 
	%    \faYoutubePlay  \space \href{https://www.youtube.com/c/tibaultreveyrand/}{www.youtube.com/c/tibaultreveyrand}
	%   
	% === OBJECTIVE ===

	%    \section{\mysidestyle Professional Objective}
	%    Improve the efficiency of the microwave and RF designers and structures. I focus on nonlinear devices at circuits level (such as HEMTs transistors) and at system level (HPA, Switches). That purpose requires an original use of an advanced RF instrumentation associated to a strong knowledge in terms of measured devices modeling.

	% === EDUCATION ===

	\section{\mysidestyle Education}
	\href{https://www.mcgill.ca/}{\textbf{McGill University}}, Montréal, QC, Canada
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] M.A., %\href{https://www.mcgill.ca/linguistics/}{
		      Linguistics%}
		      , 2025 (\textit{expected})
		\item[] B.A., %\href{https://www.cs.mcgill.ca/}{
		      Computer Science %}
		      \& %\href{https://www.mcgill.ca/linguistics/}{
		      Linguistics%}
		      , 2022
	\end{enumerate}

	\vspace{-1.2em}\rule{\textwidth}{0.4pt}

	% === RESEARCH ===

	%\section{\mysidestyle Research}


	% === WORK ===

	\section{\mysidestyle Employment}

	\textbf{Research Assistant}, \href{https://inamartinovic.com/}{Martina Martinović}, \href{https://mull-lab.org/}{MULL Lab} \hfill 2021 -- 2023
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] Igala (Volta-Niger) elicitation and annotation
		      %\item[] Annotation and grant writing and \textit{Wolof} (Atlantic–Congo)
		\item[] Build and maintain \href{https://wiki.mull-lab.org/Igala/}{Igala Wiki}, \href{https://mull-lab.org/}{MULL Lab Website}, and \href{https://github.com/dativebase/dativerf}{Dative}
	\end{enumerate}%\vspace{-0.8em}\rule{\textwidth}{0.4pt}

	\begin{comment}
	\textbf{Full Stack and Mobile Developer}, \href{https://playmind.com/}{PLAYMIND}, Montréal \hfill Mar -- Aug 2021
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] Developed~\href{https://playbox.games/\#home}{
			      PLAYBOX online management portal and Android App}
	\end{enumerate}

	\textbf{Freelance Programmer}, Montréal \hfill 2019 -- 2020
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] Web Development and Python tutoring
	\end{enumerate}

	\textbf{Junior Developer}, \href{https://gls-canada.com/en/dicom/corp/home}{GLS (\textit{fka} Dicom)}, Montréal \hfill Jan -- May 2019
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] Worked on \href{https://gls-canada.com/en/dicom/about-us/news/dicom-launches-smart4-shipping}{smart4 shipping platform} back-end
	\end{enumerate}
	\end{comment}

	\vspace{-0.9em}\rule{\textwidth}{0.4pt}


	% === FIELDWORK ===

	\section{\mysidestyle Fieldwork}

	\textbf{Igala (Volta-Niger)}, remote from Montréal to Nigeria/London \hfill May 2021 --
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] Supervision by \href{https://inamartinovic.com/}{Martina Martinović}
	\end{enumerate}


	\textbf{Wolof (Atlantic–Congo)}, Saint-Louis \& Dakar, Senegal \hfill Apr 22 -- May 28 2023
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] Supervision by \href{https://inamartinovic.com/}{Martina Martinović}
	\end{enumerate}

	\textbf{Kirundi (Bantu)}, Montréal \hfill Jan -- Jun 2022
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] As part of \href{https://jessica.lingspace.org/}{Jessica Coon}'s Field Methods class (LING 415)
		      %\item[] Taught and supervised by 
	\end{enumerate}

	\vspace{-0.9em}\rule{\textwidth}{0.4pt}

	% === AWARDS ===

	\section{\mysidestyle Awards}

	 {\textbf{Arts Undergraduate Research Award (ARIA)}. McGill University. \href{https://www.mcgill.ca/arts-internships/files/arts-internships/brandon_chaperon.pdf}{\textit{Igala Sentence-Final Particles at the edge of \textit{v}P}}. Supervised by \href{https://inamartinovic.com/}{Martina Martinović}. Summer 2022. \$5,000 (CAD)}

	\vspace{-0.8em}\rule{\textwidth}{0.4pt}

	% === PAPERS ===

	\section{\mysidestyle Proceedings\\Papers}

	 {\textbf{Chaperon, B}. (under review). A Negative Alternation: Negation Head Movement Allomorphy in Igala. \textit{Selected papers from the 54th Annual Conference on African Linguistics}. Contemporary African Linguistics: Language Science Press, 10.}

	 {\textbf{Chaperon, B}. (2023). \href{https://twpl.library.utoronto.ca/index.php/twpl/article/view/39257}{Unravelling the `not': left peripheral blocking of negation in Kirundi}. \textit{Toronto Working Papers in Linguistics}, 46(1).}%\url{https://doi.org/?...}

	\vspace{-0.8em}\rule{\textwidth}{0.4pt}

	% === PRESENTATIONS ===

	\section{\mysidestyle Presentations}

	\textbf{Conferences}
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] {2023. \textit{A negative alternation: Negation head movement allomorphy in Igala}. \href{https://uconnuecs.cventevents.com/event/aef2d257-6a6a-41b5-b83d-2c6efc60aac5/summary}{54th Annual Conference on African Linguistics (ACAL)}. University of Connecticut. June 12-14.}

		\item[] {2022. \href{https://mcling.blogs.mcgill.ca/2022/05/24/mcgill-at-ba-tom-1/}{Focusing on Negation in Kirundi}. 1st Toronto-Montreal Bantu Colloquium (Ba-TOM). University of Toronto. May 27-28.}
	\end{enumerate}

	\textbf{Reading Group/Labs}
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] {2023. \href{https://mcling.blogs.mcgill.ca/2023/01/15/mull-lab-1-17-brandon-chaperon/}{\textit{A negative alternation: Negation head movement allomorphy in Igala}}. \href{https://mull-lab.org/}{MULL Lab} meeting. McGill University. January 17.}

		\item[] {2022. \href{https://mcling.blogs.mcgill.ca/2022/03/27/mull-lab-03-30-brandon-chaperon/}{Igala's Dual Negation}. \href{https://mull-lab.org/}{MULL Lab} meeting. McGill University. March 30.}
	\end{enumerate}


	\textbf{Invited}
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] {2024. \textit{Wolof Applicatives}. \href{https://www.mcgill.ca/study/2023-2024/courses/ling-410}{LING 410 -  Structure of a Specific Language} undergraduate class for \href{https://inamartinovic.com/}{Martina Martinović}. McGill University. Feb 28.}
	\end{enumerate}

	\newpage

	\textbf{Other}
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] {2022. \href{https://www.mcgill.ca/arts-internships/files/arts-internships/brandon_chaperon.pdf}{Igala Sentence-Final Particles at the edge of \textit{v}P}. \href{https://www.mcgill.ca/arts-internships/events-0/#block-bean-annual-faculty-of-arts-undergrad}{Arts Undergraduate Research Event}. McGill University. October 18.}

		\item[] {2022. \href{https://github.com/dopestdope123/LING484-Final-Projects}{Genealogic Approach to Low-Resource Language Modelling}. \href{https://mila.quebec/en/}{MILA (Quebec AI Institute)} Poster Presentation. April 26.}

	\end{enumerate}


	\vspace{-0.6em}\rule{\textwidth}{0.4pt}

	% === COMMUNITY ===

	\section{\mysidestyle Services}

	%{\textbf{55th Annual Conference on African Linguistics (ACAL)}, co-organizer \hfill 2023 -- 2024}

	\textbf{Reading Group/Labs}
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] \href{https://mcling.blogs.mcgill.ca/category/mull-lab/}{Montreal Underdocumented Languages Linguistics Lab (MULL Lab)}\\
		      \hspace{5em}Co-organizer \hfill Winter 2023 -- 2024

		\item[] \href{https://mcling.blogs.mcgill.ca/category/syntax-semantics-group/}{McGill Syntax-Semantics Reading Group}\\
		      \indent Co-organizer \hfill Fall 2023 -- Winter 2024
	\end{enumerate}

	\textbf{Mentorship}
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] \href{https://www.mcgill.ca/cogsci/research/cogs396}{Program for Undergraduate Research in Linguistics (Pilot Project)}\\
		      \href{https://www.linkedin.com/in/emma-custer-648345252}{Emma Custer} - Syntax of Igala perfective morpheme\hfill Winter 2024
	\end{enumerate}

	\textbf{Outreach}
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] \href{https://naclo.org/}{North American Computational Linguistics Open Competition}\\
		      %McGill University site volunteer -- 
		      Training session leader \hfill Nov 2023 -- Jan 2024
	\end{enumerate}


	\textbf{Conferences \& Colloquia}
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[] \href{https://acal55.mull-lab.org/}{55th Annual Conference on African Linguistics}, McGill University\\Organizing committee \hfill Summer 2023 -- 2024

		\item[] \href{https://www.mcgill.ca/linguistics/events/colloquium-series}{McGill Linguistics Colloquium Series}\\Co-organizer\hfill Winter 2024
	\end{enumerate}

	\textbf{Reviewing}
	\begin{enumerate}[-, leftmargin=1em, topsep=4pt]
		\item[]  \href{https://acal55.mull-lab.org/}{55th Annual Conference on African Linguistics}\\
		      Abstract reviewer \hfill 2023
	\end{enumerate}

	\vspace{-0.6em}\rule{\textwidth}{0.4pt}

	% === TECHNICAL SKILLS ===

	\section{\mysidestyle Technical\\Skills}\vspace{2mm}
	\begin{description}
		\item[Operating Systems:] Linux (Arch, Debian, Ubuntu), Windows, Android
		\item[Software/Services:] Git, SVN, MySQL, MongoDB%, PostgreSQL, 
		      , Docker, Redis, RabbitMQ, Celery%, Jest Testing, 
		      , ELAN, Praat%, Ableton, FL Studio 
		\item[Languages:] Python%(PyTorch, TensorFlow, NLTK)
		      , Java, Javascript, Kotlin, PHP%(PL)
		      , SQL, NodeJS, R, C\#% (.NET)
		      %, Clojure%(script)
		      %, OCaml, Julia%(Gen)
		      , Bash, C%, \LaTeX
	\end{description}

	\vspace{-0.9em}\rule{\textwidth}{0.4pt}

	% === NATURAL LANGUAGES ===

	\section{\mysidestyle Natural Languages}\vspace{2mm}
	\begin{description}
		\item[Native:] Français (québécois)
		\item[Near-Native:] English
		\item[Intermediate:] \begin{CJK}{UTF8}{min}日本語\end{CJK}
		\item[Elementary:] Русский, español
		\item[Passive:] Kreyòl ayisyen
		\item[Fieldwork/Structure:] Igala (Volta-Niger), Kirundi (Bantu),  Wolof (Atlantic-Congo),
		      Tseltal and Tsotsil (Mayan)%, Igbo???\\
	\end{description}

	\BgThispage

\end{resume}
\end{document}